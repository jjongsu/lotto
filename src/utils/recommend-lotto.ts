...
const LOTTO_NUMBERS = Array.from({ length: LOTTO_MAX }, (_, index) => index + LOTTO_MIN);
...
const pickWeightedNumber = (pool: number[], frequencyMap: FrequencyMap, rng: () => number): number => {
    ...
    if (totalWeight <= 0) {
        return pool[Math.floor(rng() * pool.length)] ?? LOTTO_MIN;
    }
    ...
    return pool[pool.length - 1] ?? LOTTO_MIN;
};
...
const toRecommendedSet = (numbers: number[]): RecommendedSet => {
    ...
    if (normalized.length !== LOTTO_PICK_COUNT) {
        const fallback = shuffleNumbers(LOTTO_NUMBERS, Math.random).slice(0, LOTTO_PICK_COUNT).sort((a, b) => a - b);
...
const generateWeightedSet = (frequencyMap: FrequencyMap, rng: () => number): RecommendedSet => {
    ...
    while (picked.size < LOTTO_PICK_COUNT) {
        const pool = LOTTO_NUMBERS.filter((number) => !picked.has(number));
...
const generateUniformSet = (rng: () => number): RecommendedSet => {
    const set = shuffleNumbers(LOTTO_NUMBERS, rng).slice(0, LOTTO_PICK_COUNT);
